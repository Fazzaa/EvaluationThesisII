import {data} from "./data/dataset_part_two";
...
type AnswerType = "Impossibile" | "Improbabile" | "Mediamente probabile" | "Molto probabile";
...
const firebaseConfig = {
    apiKey: "AIzaSyDh_iuesG6NZ8YAZ6mVTGnYIlbd9jZBfH0",
    authDomain: "testtesi2.firebaseapp.com",
    projectId: "testtesi2",
    storageBucket: "testtesi2.appspot.com",
    messagingSenderId: "81327731379",
    appId: "1:81327731379:web:aa208cca6e636cd8d2f6c6"
  };

// Initialize Firebase
const app = initializeApp(firebaseConfig);
const firstore = getFirestore(app);
const col = collection(firstore, "annotations_part_two") as CollectionReference<DataType>;
...
function App() {
    ...
    const next = async (newAnswers: AnswerType[]) => {
        if (!isLoading) {
            setIsLoading(true);
            ...
            if (dataset && i < dataset.length) {
                ...
                const docRef = doc(firstore, "annotations_part_two/" + name);
        ...
        !ready ?
            ...
            : <div>
                <div className="text-center justify-center mt-12">
                    {<MadeByMe/>}
                    <h1 className="text-4xl font-bold">Valutazione seconda parte</h1>
                    <p className="text-xl mt-6"
                       id="line"><span
                        className="">{dataset ? dataset[i] : ""}</span>
                ...
                <div className="flex justify-center mt-8">
                    <button id="button-1"
                            onClick={() => onClick("Impossibile")}
                            className="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-full transition duration-300 ease-in-out transform hover:-translate-y-1 hover:scale-110 shadow-lg disabled:bg-gray-500"
                            disabled={isLoading}>
                        Impossibile
                    </button>
                    <button id="button-2"
                            onClick={() => onClick("Improbabile")}
                    ...
                    <button id="button-3"
                        onClick={() => onClick("Mediamente probabile")}
                    ...
                    <button id="button-4"
                        onClick={() => onClick("Molto probabile")}
                        className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-full ml-4 transition duration-300 ease-in-out transform hover:-translate-y-1 hover:scale-110 shadow-lg disabled:bg-gray-500"
                        disabled={isLoading}>
                    Molto probabile
                    </button>
                </div>
